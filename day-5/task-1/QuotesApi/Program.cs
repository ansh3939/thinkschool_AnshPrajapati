...
// Bound once here so the JwtBearer scheme below (configured before the DI container is
// built) has the issuer/audience/signing key it needs immediately. The same section is
// also registered via Configure<JwtOptions> so anything resolved through DI - JwtTokenService,
// or an IOptionsSnapshot<JwtOptions>/IOptionsMonitor<JwtOptions> added later - gets it too.
...
{
    return Results.Ok(await db.Quotes.OrderBy(quote => quote.Id).ToListAsync());
});
